...
class REPL:
    ...
    def Count(self, index_provided = None):
        ...
        if not index_provided:
            data = REPL.output_data[-1]
        else:
            data = index_provided
        self.output_data.append(len(data))
        print(len(data))
...
def arg_parse(arg: str):
    if arg == 'self':
        arg = REPL.output_data
    elif arg.startswith("#"):
        arg = arg.strip('#')
        if arg in globals():
            arg = globals()[arg]
        elif arg in dir(builtins):
            arg = getattr(builtins, arg)
        elif arg in dir(REPL):
            arg = getattr(REPL, arg)
        else:
            arg = eval(arg)
    elif arg.startswith('^'):
        # breakpoint()
        arg = REPL.output_data[int(arg.strip('^'))]
    else:
        arg = eval(arg)
    return arg
...
class InteractiveConsole(InteractiveInterpreter):
    ...
    def push(self, line):
        ...
        """
#        breakpoint()
        if line.startswith('quit') or line.startswith('q'):
            raise EOFError
        try:
            commands = line.split(',')
            # breakpoint()
            for command in commands:
                ...
                args = [arg_parse(arg) for arg in args]
                if '.' in func:
                    parts = func.split('.')
