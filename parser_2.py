...
# -*- coding: utf-8 -*-
"""
Created on Tue Mar  3 12:03:27 2020
...
class REPL:
    ...
    def Open(cls, filename):
        ...
        with suppress(FileNotFoundError):
            ...
            data = fileobj.read()
            fileobj.close()
            cls.output_data.append(data)
            cls.index_counter += 1
            return ascii(f"#{cls.index_counter - 1}: {fileobj!r}")

    @classmethod
    def Filter(cls, token, index_provided = None):
        ...
        try:
            ...
            index_provided = -1
        breakpoint()
        data = cls.output_data[index_provided]
        filtered = [line for line in data.split('\n') if token in line]
        cls.output_data.append(filtered)
        cls.index_counter += 1
        return ascii(f"#{cls.index_counter - 1}: {filtered}")

    @classmethod
    def Count(cls, index_provided = None):
...
#def parser(command):
#	paren_id = command.find('('):157

#	if paren_id != -1:
#		parse(command[paren_id + 1])
#	elif paren_id.count('(') == 1 and paren_id == 0:
#		if not command[-1] == ')':
#			raise SyntaxError("Imbalanced parenthesis, at least one parenthesis not closed.")
#		else:
#			parse(command.replace('(/)'))
#	else:
#		return command
...
#        breakpoint()
        ...
        for command in commands:
        	func, *args = command.strip().split()
        	args = [eval(arg) for arg in args]
        	if func in globals():
        	    func = globals()[func]

        	elif func in dir(REPL):
        	    func = getattr(REPL, func)
        	elif func in dir(builtins):
        	    func = vars(builtins)[func]
        	    args = REPL.output_data[-1:] if not len(args) else args
#        	expr = f"self.obj.{func}({''.join(args)})"
#        	    breakpoint()
        	    op = func(args)
        	    REPL.output_data.append(op)
        	    self.resetbuffer()
        	    return False
#			else:
#				try:
